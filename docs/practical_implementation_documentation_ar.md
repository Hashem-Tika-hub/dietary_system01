...
تجمع الاختبارات بين اختبارات الوحدة والمنطق واختبارات التكامل. في آخر تحقق محلي موثق، نجح **53 اختبارًا** بعد إضافة حالات حافة لمدخلات ملف المستخدم، وJWT، وتصفية الحساسيات المتعددة مع بيانات كتالوج ناقصة، وقيود قاعدة البيانات المباشرة، وتسجيل تفاعل Flutter عبر `fdc_id`، وملخص الاستهلاك اليومي الذي يستبعد سجلات الأيام والمستخدمين الآخرين. هذه النتيجة تثبت سلامة السلوك المتوقع في السيناريوهات المختبرة، لكنها لا تثبت أثرًا صحيًا سريريًا أو رضا مستخدمين حقيقيين.
...
أضيف اختباران للتكامل في `test_user_meal_log_api.py`: الأول يتحقق من جمع سجلات اليوم فقط واستبعاد سجل مستخدم آخر ويوم سابق، والثاني يتحقق من حالة اليوم الفارغ. أضيف كذلك اختبار وحدة لنموذج `DailyNutritionProgress` واختبارا Widget للـDashboard، يغطيان حالة وجود وجبات وحالة السجل الفارغ. اجتازت اختبارات API محليًا **53 اختبارًا**. تتطلب اختبارات Flutter تنفيذ `flutter analyze` و`flutter test` على جهاز تطوير يحتوي Flutter SDK قبل بناء APK.
