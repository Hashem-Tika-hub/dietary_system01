...
تجمع الاختبارات بين اختبارات الوحدة والمنطق واختبارات التكامل. في آخر تحقق محلي موثق، نجح **43 اختبارًا** بعد إضافة 11 اختبارًا لحالات الحافة في مدخلات ملف المستخدم، وJWT، وتصفية الحساسيات المتعددة مع بيانات كتالوج ناقصة. هذه النتيجة تثبت سلامة السلوك المتوقع في السيناريوهات المختبرة، لكنها لا تثبت أثرًا صحيًا سريريًا أو رضا مستخدمين حقيقيين.

| الملف أو نوع الاختبار | ما يتحقق منه |
|---|---|
| `test_database_migrations.py` | إنشاء المخطط وترقيته والتحقق من تغيرات غير مرحلة. |
...
| `test_api_security_edge_cases.py` | رفض العمر والوزن والطول ومستوى النشاط الصفرية أو السالبة، ورفض JWT المفقود أو المنتهي، وعدم حفظ أي قيمة غير صالحة. |
