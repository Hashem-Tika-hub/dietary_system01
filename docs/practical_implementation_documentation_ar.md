...
**حالة الشفرة الموثقة:** فرع `main` بعد دمج PR #5، متضمنًا Dashboard وتتبع الاستهلاك اليومي حتى commit `249fde3`
...
تعالج الدالة `build_food_cluster_map()` البيانات المكررة والقيم غير الرقمية، وتستبدل القيم المفقودة بوسيط العمود ثم بالصفر إن بقي نقص. تطبق `StandardScaler` قبل K-Means، لأن اختلاف وحدات ومجالات المغذيات قد يجعل الخاصية ذات المدى الأكبر تهيمن على المسافة. يستخدم التنفيذ الحالي `random_state=42` و`n_init=20` و`n_clusters=6` كقيمة تشغيل بداية قابلة لإعادة الإنتاج؛ ولا يساوي هذا الإعداد ادعاءً بأنه الأفضل مطلقًا.
...
كانت القيمة `K=6` إعداد بداية، ثم نُفذ التقييم فعليًا على 456 صنفًا من `foods_clean.csv` لاختبار K من 2 إلى 12. أنتج السكربت `scripts/evaluate_food_clusters.py` جدول النتائج والرسم والتقرير في `reports/kmeans_cluster_evaluation_*`. أعطت الرتبة المركبة غير الطبية أفضلية أولية لـ`K=8`، بينما كانت أعلى Silhouette عند `K=4` وأقل Davies–Bouldin عند `K=7`. لذلك تبقى `K=6` إعداد التشغيل الحالي لحين فحص نوعي للمجموعات واختبار تنوع الخطة end-to-end؛ لا يُغيّر الإعداد اعتمادًا على رقم واحد فقط.
...
> حفظ المشروع الآن منحنيات Elbow وSilhouette وCalinski–Harabasz وDavies–Bouldin وجدول المقاييس في `reports/kmeans_cluster_evaluation.png` و`reports/kmeans_cluster_evaluation_metrics.csv`. يبرر القرار النهائي بدمج هذه الأدلة مع قابلية فهم المجموعات وتحسن تنوع الخطة من دون خرق قيود الأهلية، لا بتقديم قيمة K كحقيقة طبية مطلقة. تشرح وثائق scikit-learn أن K-Means يتطلب تحديد عدد المجموعات مسبقًا؛ لذلك يجب توثيق الاختيار ومراجعته مع تطور الكتالوج [5].
...
تجمع الاختبارات بين اختبارات الوحدة والمنطق واختبارات التكامل. في آخر تحقق محلي موثق، نجح **59 اختبارًا** بعد إضافة حالات حافة لمدخلات ملف المستخدم، وJWT، وتصفية الحساسيات المتعددة مع بيانات كتالوج ناقصة، وقيود قاعدة البيانات المباشرة، وتسجيل تفاعل Flutter عبر `fdc_id`، وملخص الاستهلاك اليومي الذي يستبعد سجلات الأيام والمستخدمين الآخرين، وثبات توصيات CF والتفضيل السلبي والتقييم المكرر. هذه النتيجة تثبت سلامة السلوك المتوقع في السيناريوهات المختبرة، لكنها لا تثبت أثرًا صحيًا سريريًا أو رضا مستخدمين حقيقيين.
...
| `test_feedback_collaborative_filter.py` | Cold Start، بوابة الجاهزية، العناصر غير المرئية فقط، ثبات النتائج، عدم اقتراح دعم سلبي، وحفظ آخر قيمة عند تكرار التفاعل. |
| `test_food_feedback_api.py` | تفاعل صريح بطريقة upsert، والتحقق من `food_id` و`fdc_id` ورفض المعرفات المتعارضة. |
...
| `test_user_meal_log_api.py` | ملكية المستخدم وCRUD لسجل الوجبات والملخص، وجمع تقدم اليوم بدقة مع استبعاد الأيام والمستخدمين الآخرين. |
...
## 16. Dashboard وتتبع سجل الوجبات الفعلي

### 16.1 الغرض وسير العمل المنفذ

أضيفت Dashboard رئيسية لتطبيق Flutter تربط الواجهة بالبيانات المتاحة فعليًا بدل عرض مؤشرات شكلية. تبدأ الشاشة بالهدف اليومي وحالة التخصيص، ثم تعرض بطاقة **«متابعة اليوم»** التي تقارن الاستهلاك المسجل بالهدف الحالي. عند اختيار المستخدم توصية، يضغط **«سجّل كوجبة مأكولة»** ويؤكد العملية؛ يحفظ التطبيق الحصة والمغذيات الظاهرة في `POST /users/meal-logs`، ثم يعيد تحميل التقدم اليومي. يبقى هذا المسار منفصلًا عن التفضيلات الصريحة حتى لا يعد الاستهلاك إعجابًا ضمنيًا.

| الجزء | مصدر البيانات | السلوك الظاهر للمستخدم |
|---|---|---|
| الأهداف اليومية | `GET /users/nutrition-targets` | السعرات والبروتين والكربوهيدرات والدهون المخططة. |
| الاستهلاك اليومي | `GET /users/meal-logs/daily-progress` | المستهلك والمتبقي والنسبة وعدد الوجبات المسجلة. |
| تسجيل وجبة | `POST /users/meal-logs` | حفظ الحصة والمغذيات ثم تحديث Dashboard. |
| تفضيل الطعام | `POST /users/food-feedback` | إعجاب أو حفظ أو عدم إعجاب أو عدم اهتمام، مستقل عن السجل. |

يعزل مسار التقدم اليومي مستخدم JWT الحالي واليوم المطلوب فقط، ثم يعيد `target` و`consumed` و`remaining` و`progress_ratio` لكل مغذٍ. يمكن أن تكون قيمة المتبقي سالبة عند تجاوز الهدف التقريبي؛ يقيد Flutter شريط العرض بين 0 و1، لكنه لا يخفي مقدار التجاوز. عند عدم وجود سجلات، تعرض البطاقة بوضوح أن اليوم لا يحتوي على وجبات مسجلة، ولا تدعي وجود سعرات مستهلكة.

### 16.2 الاختبار وحالة التنفيذ

أضيف اختباران للتكامل في `test_user_meal_log_api.py`: الأول يتحقق من جمع سجلات اليوم فقط واستبعاد سجل مستخدم آخر ويوم سابق، والثاني يتحقق من حالة اليوم الفارغ. أضيف كذلك اختبار وحدة لنموذج `DailyNutritionProgress` واختبارا Widget للـDashboard، يغطيان حالة وجود وجبات وحالة السجل الفارغ. اجتازت اختبارات API محليًا **59 اختبارًا** بعد إضافة ست حالات وحدة جديدة لـCollaborative Filtering. تتطلب اختبارات Flutter تنفيذ `flutter analyze` و`flutter test` على جهاز تطوير يحتوي Flutter SDK قبل بناء APK.

### 16.3 تحسين CF بواسطة الإشارات الضمنية

سجل الوجبة دليل استخدام ضمني لا تقييم صريح. لذلك لا يفعل النظام الحالي CF من السجلات وحدها؛ يظل `Content-Based Filtering` المسار الافتراضي، ولا يفعل CF إلا عند تحقق بوابة التفاعلات الصريحة الحقيقية. تقترح خارطة التطوير إضافة تعزيز محدود ومبني على موافقة المستخدم لتكرار السجل وحداثته، ثم قياسه زمنيًا قبل مزجه مع CBF وCF الصريح. يظل التفضيل السلبي الصريح أعلى أولوية، وتستمر الحساسية والقيود الصلبة قبل جميع الدرجات. يتوفر التفصيل العلمي وخطة التنفيذ والاختبارات في [`implicit_feedback_cf_improvement_ar.md`](implicit_feedback_cf_improvement_ar.md) [7].

## 17. القيود الواقعية وخطة التحسين
...
[7] [Hu, Koren, and Volinsky — Collaborative Filtering for Implicit Feedback Datasets](https://doi.org/10.1109/ICDM.2008.22) — مرجع أكاديمي يوضح اختلاف الثقة بين الإشارات الضمنية وغياب دليل مباشر على عدم التفضيل.
