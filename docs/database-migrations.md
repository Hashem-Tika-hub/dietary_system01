...
> الترحيل ينشئ المخطط فقط. لمزامنة كتالوج الطعام في قاعدة تشغيل جديدة، شغّل أداة التهيئة بعد نجاح Alembic. لا تنسخ نسخة SQLite المرجعية إلى موقع قاعدة تشغيل؛ الأداة تنقل جداول الكتالوج فقط إلى `DATABASE_URL`.
...
في Docker الإنتاجي، ينفّذ `docker/entrypoint.sh` الخطوتين تلقائيًا بالترتيب نفسه بعد أن تصبح PostgreSQL سليمة.
...
| إنتاج | `postgresql+psycopg://...` داخل Docker Compose | قاعدة تشغيل API، غير متعقبة في Git. |
...
كل تغيير للمخطط يكون في revision جديد قابل للمراجعة؛ لا تعدّل revision طُبق في بيئة مشتركة. أبقِ `DATABASE_URL` في متغيرات البيئة أو منصة النشر، ولا تضع بيانات الاتصال أو المفاتيح في `alembic.ini` أو المستودع. لا ترفع `.env` أو قاعدة تشغيل SQLite أو نسخة PostgreSQL أو نسخ المستخدمين الاحتياطية إلى Git. الاستثناء المقصود هو `data/catalog/food_catalog_reference.sqlite3`، وهو ملف كتالوج فقط للمراجعة وإعادة التهيئة.
